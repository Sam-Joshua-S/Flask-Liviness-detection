...
def login():
    ...
    vid.set(cv2.CAP_PROP_HW_ACCELERATION, True)
    if request.method == 'POST':
        ...
        engine.runAndWait()
        flag , face_embedding = liveness(vid)
...
def sign_up():
    ...
    if request.method == 'POST':
        ...
        if email_exits:
            ...
            new_User = User(email=email,username=username,password=bcrypt.hashpw(str(password1.encode()), bcrypt.gensalt()),face_embedding=str(face_embedding),hand_model=str(hand_module))
